...
def test_price():
    ...
    BuyBack.setBurnWallet(deployer,{'from':deployer,'gas_price':GasPrice})
    BuyBack.setApproval({'from':deployer,'gas_price':GasPrice})
    BuyBack.buyBackAndBurn(1e20,{'from':deployer,'gas_price':GasPrice})
    assert(interface.IERC20(token1).balanceOf(BuyBack.burnWallet.call()) > 0)
    assert(False)
