...
contract ITokenPriceFeedHelperV2_ARB is IPriceFeedHelper, PausableGuardian_0_8 {
    ...
    mapping(address => address) public tokenInfo;

    function updateTokenInfo(address token, address priceFeed) onlyGuardian public {
        tokenInfo[token] = priceFeed;
    }
    
    function latestAnswer(address token) public view returns (uint256) {       
        return (uint256(IPriceFeedsExt(tokenInfo[token]).latestAnswer()) * IToken(token).tokenPrice()) / 1e18;
    }
}
