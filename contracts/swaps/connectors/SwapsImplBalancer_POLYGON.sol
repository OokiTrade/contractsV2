...
contract SwapsImplBalancer_POLYGON is State, ISwapsImpl {
  ...
  function dexAmountOut(
    ...
    (
      IBalancerVault.BatchSwapStep[] memory swapParams,
      address[] memory tokens
    ) = abi.decode(payload, (IBalancerVault.BatchSwapStep[], address[]));
  ...
  function dexAmountIn(
    ...
    (
      IBalancerVault.BatchSwapStep[] memory swapParams,
      address[] memory tokens
    ) = abi.decode(route, (IBalancerVault.BatchSwapStep[], address[]));
    ...
    returns (uint256 sourceTokenAmountUsed, uint256 destTokenAmountReceived)
  {
    if (requiredDestTokenAmount == 0) {
      ...
      require(
        tokens[0] == sourceTokenAddress &&
          tokens[tokens.length - 1] == destTokenAddress,
        'invalid tokens'
      );
      ...
      if (requiredDestTokenAmount < minSourceTokenAmount) {
        swapParams[swapParams.length - 1].amount -=
          minSourceTokenAmount -
          requiredDestTokenAmount;
