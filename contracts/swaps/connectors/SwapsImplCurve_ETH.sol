...
contract SwapsImplCurve_ETH is State, ISwapsImpl {
	...
	bytes4 public constant GetDyUnderlyingSig = bytes4(keccak256("get_dy_underlying(uint256,uint256,uint256)"));
    function dexSwap(
    ...
    function _getAmountOut(uint256 amountIn, bytes memory path)
        ...
        returns (uint256)
    {
        (bytes4 sig, address curvePool, uint128 tokenIn, uint128 tokenOut) = abi.decode(
            path,
            (bytes4, address, uint128, uint128)
        );
		uint256 amountOut;
