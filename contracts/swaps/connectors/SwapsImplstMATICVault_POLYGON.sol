...
contract SwapsImplstMATICVault_POLYGON is State, ISwapsImpl {
  ...
  IBalancerHelpers public constant HELPER =
    IBalancerHelpers(0x239e55F427D44C3cc793f49bFB507ebe76638a2b);
    ...
    returns (uint256 destTokenAmountReceived, uint256 sourceTokenAmountUsed)
  {
    ...
    require(
      (sourceTokenAddress == WMATIC || sourceTokenAddress == address(VAULT)) &&
        (destTokenAddress == WMATIC || destTokenAddress == address(VAULT)),
      'unsupported tokens'
    );
  ...
  function dexAmountOut(
    ...
    if (amountIn != 0) {
      (uint256 limiter, address sourceToken) = abi.decode(
        payload,
        (uint256, address)
      );
      if (sourceToken == WMATIC) {
        ...
        addrs[0] = WMATIC;
        addrs[1] = STMATIC;
        IBalancerHelpers.JoinPoolRequest memory req = IBalancerHelpers
          .JoinPoolRequest({
            assets: addrs,
            maxAmountsIn: values,
        ...
        addrs[0] = WMATIC;
        addrs[1] = STMATIC;
        IBalancerHelpers.ExitPoolRequest memory req = IBalancerHelpers
          .ExitPoolRequest({
            assets: addrs,
    ...
    returns (uint256 sourceTokenAmountUsed, uint256 destTokenAmountReceived)
  {
    ...
    if (sourceTokenAddress == WMATIC) {
      ...
      addrs[0] = WMATIC;
      addrs[1] = STMATIC;
      IBalancerVault.JoinPoolRequest memory req = IBalancerVault
        .JoinPoolRequest({
          assets: addrs,
          maxAmountsIn: values,
      ...
      destTokenAmountReceived = VAULT.deposit(
        IERC20(ASSET).balanceOf(_thisAddress) - destTokenAmountReceived,
        receiverAddress
      );
    } else {
      minSourceTokenAmount = VAULT.redeem(
        minSourceTokenAmount,
        _thisAddress,
        _thisAddress
      );
      ...
      addrs[0] = WMATIC;
      addrs[1] = STMATIC;
      IBalancerVault.ExitPoolRequest memory req = IBalancerVault
        .ExitPoolRequest({
          assets: addrs,
          minAmountsOut: values,
      ...
      IBalancerVault(_vault).exitPool(
        POOLID,
        _thisAddress,
        payable(receiverAddress),
        req
      );
      destTokenAmountReceived =
        IERC20(WMATIC).balanceOf(receiverAddress) -
        destTokenAmountReceived;
    }
  }
}
