...
contract Voting is Common {
  ...
  function votingBalanceOf(
    ...
    (, , , uint256 startBlock, , , , , , ) = GovernorBravoDelegateStorageV1(
      governor
    ...
    return
      _votingBalanceOf(account, _proposalState[proposalId], startBlock - 1);
  }
  ...
  function _votingBalanceOf(
    ...
    if (
      _voteDelegator._isPaused(_voteDelegator.delegate.selector) ||
      _voteDelegator._isPaused(_voteDelegator.delegateBySig.selector)
    ) {
