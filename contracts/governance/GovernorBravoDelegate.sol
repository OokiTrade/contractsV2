...
contract GovernorBravoDelegate is GovernorBravoDelegateStorageV1, GovernorBravoEvents {
  ...
  uint256 public constant MIN_PROPOSAL_THRESHOLD = 0.5e18; // 0.5% of OOKI

  /// @notice The maximum setable proposal threshold
  uint256 public constant MAX_PROPOSAL_THRESHOLD = 2e18; // 2% of OOKI
  ...
  uint256 public constant MIN_QUORUM_PERCENTAGE = 2e18; // 2% of total OOKI supply

  /// @notice The max setable quorum percentage
  uint256 public constant MAX_QUORUM_PERCENTAGE = 6e18; // 6% of total OOKI supply

  /// @notice The maximum number of actions that can be included in a proposal
  uint256 public constant proposalMaxOperations = 100; // 100 actions
  ...
  function initialize(
    ...
    uint256 proposalThresholdPercentage_,
    ...
    require(
      proposalThresholdPercentage_ >= MIN_PROPOSAL_THRESHOLD && proposalThresholdPercentage_ <= MAX_PROPOSAL_THRESHOLD,
  ...
  function cancel(uint256 proposalId) external {
    ...
    Proposal storage proposal = proposals[proposalId];
    require(
      msg.sender == proposal.proposer || staking.votingBalanceOfNow(proposal.proposer) < proposalThreshold() || msg.sender == guardian,
  ...
  function quorumVotes() public view returns (uint256 votes) {
    uint256 totalSupply = IERC20(0x0De05F6447ab4D22c8827449EE4bA2D5C288379B).totalSupply(); // OOKI
   ...
   */
  function castVotes(uint256[] calldata proposalIds, uint8[] calldata supportVals) external {
   ...
   */
  function __setVotingDelay(uint256 newVotingDelay) external {
   ...
   */
  function __setVotingPeriod(uint256 newVotingPeriod) external {
   ...
   */
  function __setQuorumPercentage(uint256 newQuorumPercentage) external {
   ...
   */
  function __setProposalThresholdPercentage(uint256 newProposalThresholdPercentage) external {
    require(msg.sender == admin, "GovernorBravo::__setProposalThreshold: admin only");
    require(
      newProposalThresholdPercentage >= MIN_PROPOSAL_THRESHOLD && newProposalThresholdPercentage <= MAX_PROPOSAL_THRESHOLD,
      "GovernorBravo::__setProposalThreshold: invalid proposal threshold"
    );
    uint256 oldProposalThresholdPercentage = proposalThresholdPercentage;
