...
contract LoanTokenization is ReentrancyGuardV4, Ownable {
...
contract AdvancedTokenV4Storage is LoanTokenStorage {
...
contract AdvancedTokenV4 is AdvancedTokenV4Storage {
    using SafeMath for uint256;
...
contract LoanTokenLogicV4 is AdvancedTokenV4, OracleNotifierInterface {
    using SafeMath for uint256;
    ...
    function()
        external
    {
        revert("Please visit  https://ooki.com and migrate to new IToken");
    }
    ...
    function migrate(IToken _newToken, address _to, uint256 balance) external onlyOwner {
        _newToken.mint(
            _to, balance
        );
    }

    function setApprovals(ERC20 _token, address _spender, uint256 _amount) external onlyOwner {
