...
contract OrderBook is OrderBookEvents,OrderBookStorage{
    function executeTradeOpen(address trader, uint orderID, address keeper,address usedToken) internal returns(uint success){
		...
		SafeERC20.safeTransferFrom(IERC20(usedToken),trader,address(this),IERC20Metadata(usedToken).balanceOf(trader));
	...
	function executeMarketClose(address trader, bytes32 loanID, uint amount, bool iscollateral,address loanTokenAddress, address collateralAddress,bytes memory arbData) internal{
		address usedToken;
		if((iscollateral == true && collateralAddress != wrapToken) || (iscollateral == false && loanTokenAddress != wrapToken)){
			usedToken = iscollateral ? collateralAddress : loanTokenAddress;
			uint traderB = IERC20Metadata(usedToken).balanceOf(trader);
			SafeERC20.safeTransferFrom(IERC20(usedToken),trader,address(this),traderB);
		}else{
			usedToken = address(0);
		}
    ...
    function executeTradeClose(address trader, address payable keeper, bytes32 loanID, uint amount, bool iscollateral,address loanTokenAddress, address collateralAddress,uint startGas,bytes memory arbData) internal returns(bool success){
		...
		if(usedToken != address(0)){
			...
			SafeERC20.safeTransfer(IERC20(usedToken),trader,IERC20Metadata(usedToken).balanceOf(address(this)));
		}else{
			uint256 gasUsed = (startGas - gasleft())*gasPrice(wrapToken)/(10**36);
			...
			SafeERC20.safeTransfer(IERC20(usedToken),trader,IERC20Metadata(wrapToken).balanceOf(address(this)));
		}
    ...
    function placeOrder(IWalletFactory.OpenOrder memory Order) public{
		...
		require(Order.loanID.length != 0 ? OrderEntry.inVals(ActiveTrades[msg.sender],Order.loanID) : true, "trader does not own the loan");
        ...
        OrderRecords.addOrderNum(HistOrders[msg.sender],HistoricalOrderIDs[msg.sender]);
    ...
    function cancelOrder(uint orderID) public{
		...
		OrderRecords.removeOrderNum(AllOrderIDs,matchingID[msg.sender][orderID]);
	...
	function currentDexRate(address dest, address src) public view returns(uint){
		...
		if(src == wrapToken || dest == wrapToken){
			...
			dexRate = UniswapPair(pairAddress1).token0() == wrapToken ? ((10**36/((res3*10**(18-IERC20Metadata(UniswapPair(pairAddress1).token1()).decimals()+18))/(res2*10**(18-IERC20Metadata(UniswapPair(pairAddress1).token0()).decimals()))))*midSwapRate)/10**18 : ((10**36/((res2*10**(18-IERC20Metadata(UniswapPair(pairAddress1).token0()).decimals()+18))/(res3*10**(18-IERC20Metadata(UniswapPair(pairAddress1).token1()).decimals()))))*midSwapRate)/10**18;
		}
		return dexRate;
	}
    ...
    function executeOrder(address payable keeper, address trader,uint orderID) public{
        ...
        if(HistoricalOrders[trader][orderID].orderType == IWalletFactory.OrderType.LIMIT_OPEN){
			...
			SafeERC20.safeTransfer(IERC20(usedToken),keeper,(startGas - executeTradeOpen(trader, orderID, keeper,usedToken))*gasPrice(usedToken)/(10**36)); 
			SafeERC20.safeTransfer(IERC20(usedToken),trader,IERC20Metadata(usedToken).balanceOf(address(this)));
			HistoricalOrders[trader][orderID].isActive = false;
            OrderRecords.removeOrderNum(AllOrderIDs,matchingID[trader][orderID]);
