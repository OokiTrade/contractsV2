...
contract HelperImpl is Ownable {
  address public constant bZxProtocol =
  ...
  function totalSupply(
    IERC20[] calldata tokens
  ) public view returns (uint256[] memory totalSupply) {
  ...
  function profitOf(
    IToken[] calldata tokens,
    address wallet
  ) public view returns (int256[] memory profits) {
  ...
  function marketLiquidity(
    IToken[] calldata tokens
  ) public view returns (uint256[] memory liquidity) {
    liquidity = new uint256[](tokens.length);
    for (uint256 i = 0; i < tokens.length; i++) {
      liquidity[i] = IERC20(tokens[i].loanTokenAddress()).balanceOf(
        address(tokens[i])
      );
    }
  }
  ...
  function reserveDetails(
    IToken[] calldata tokens
  ) public view returns (ReserveDetail[] memory reserveDetails) {
    ...
    for (uint256 i = 0; i < tokens.length; i++) {
      ...
      reserveDetails[i].torqueBorrowInterestRate = tokens[i]
        .nextBorrowInterestRate(0);
      reserveDetails[i].vaultBalance = IERC20(tokens[i].loanTokenAddress())
        .balanceOf(bZxProtocol);
    }
  }
  ...
  function assetRates(
    ...
    for (uint256 i = 0; i < tokens.length; i++) {
      ...
      if (sourceAmounts[i] != 0) {
        ...
        assetRates[i].destAmount =
          assetRates[i].destAmount /
          assetRates[i].precision;
      }
    }
  }
    ...
    )
  {
    IToken iToken = IToken(
      IBZx(bZxProtocol).underlyingToLoanPool(loanTokenAddress)
    );
    if (borrowAmount != 0) {
      if (borrowAmount <= IERC20(loanTokenAddress).balanceOf(address(iToken))) {
        ...
        (IBZx.LoanParams memory loanParam, ) = IBZx(bZxProtocol)
          .getDefaultLoanParams(loanTokenAddress, collateralTokenAddress, true);
        return
  ...
  function getRequiredCollateralByParams(
    IBZx.LoanParams memory loanParamsLocal,
  ...
  function getBorrowAmount(
    ...
    if (marginAmount != 0) {
      ...
      if (loanToken == collateralToken) {
        borrowAmount =
          (collateralTokenAmount * (WEI_PERCENT_PRECISION)) /
          (marginAmount);
      } else {
        (uint256 sourceToDestRate, uint256 sourceToDestPrecision) = IPriceFeeds(
          IBZx(bZxProtocol).priceFeeds()
