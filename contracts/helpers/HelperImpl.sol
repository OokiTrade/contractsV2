...
contract HelperImpl is Ownable {
  address public immutable bZxProtocol;
  ...
  constructor(address protocol, address weth) {
    bZxProtocol = protocol;
    wethToken = weth;
  }
  ...
  function getBorrowAmountByParams(IBZx.LoanParams memory loanParamsLocal, uint256 collateralTokenAmount) public view returns (uint256 borrowAmount) {
    // IBZx.LoanParams memory loanParamsLocal = IBZx(bZxProtocol).loanParams(loanParamsId);
    return
      getBorrowAmount(
        loanParamsLocal.loanToken,
