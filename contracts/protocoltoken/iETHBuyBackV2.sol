...
interface iETHBuyBackV1 {
  ...
  function iETHSold() external view returns (uint256);

  function vBZRXBought() external view returns (uint256);
}
...
contract iETHBuyBackV2 is Ownable {
  ...
  function convert(uint256 _tokenAmount) public {
    ...
    vBZRX.transfer(msg.sender, buyAmount);
  ...
  function transferV1Ownership(address _newOwner) public onlyOwner {
  ...
  function withdrawVBZRX(uint256 _amount) public onlyOwner {
    ...
    if (_amount != 0) {
      vBZRX.transfer(msg.sender, _amount);
    }
  }

  function withdrawIETH(uint256 _amount) public onlyOwner {
    ...
    if (_amount != 0) {
      iETH.transfer(msg.sender, _amount);
    }
  }
  ...
  function iETHSwapRateWithCheck(address _buyer) public view returns (uint256) {
  ...
  function whitelist(address _buyer) public view returns (uint256) {
