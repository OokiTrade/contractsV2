...
contract BZRXVestingToken is CheckpointingToken, Ownable {
  ...
  function initialize() external {
    ...
    BZRX.transferFrom(msg.sender, address(this), startingBalance_);
  ...
  function vestedBalanceOf(address _owner) public view returns (uint256) {
    ...
    if (lastClaim < _getTimestamp()) {
      return _totalVested(balanceOfAt(_owner, _getBlockNumber()), lastClaim);
    }
  }
  ...
  function vestingBalanceOf(address _owner) public view returns (uint256 balance) {
    ...
    if (balance != 0) {
      uint256 lastClaim = lastClaimTime_[_owner];
      if (lastClaim < _getTimestamp()) {
        balance = sub(balance, _totalVested(balance, lastClaim));
      }
    }
  }
  ...
  function totalUnclaimed() external view returns (uint256) {
    return sub(_totalVested(startingBalance_, 0), totalClaimed);
  }
