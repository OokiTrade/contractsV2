...
contract BridgeCollectedFees is Upgradeable_0_8 {
	...
	function initialize(
		...
		uint256 MaxFee,
		uint256 minBridge,
		uint256 minBondFee) public onlyOwner
	{
	...
	function bridgeUSDC() public
	{
		...
		Bridge.swapAndSend(
			...
			balance*BonderFeePercent/WEI_PERCENT_PRECISION>MinBonderFee?balance*BonderFeePercent/WEI_PERCENT_PRECISION:MinBonderFee, //MAX(minimmum bonder fee, 0.25% of bridge amount)
			balance-balance*MaxSlippage/WEI_PERCENT_PRECISION, //uses a defined max slippage for USDC to hUSDC
			block.timestamp,
			IHopExchange(HopExchange).calculateSwap(0,1,balance)-MaxFeeForBridge, //MaxFeeForBridge >= bonder fee + tx cost on-chain
			block.timestamp+ONE_WEEK); //1 week deadline for bridge
