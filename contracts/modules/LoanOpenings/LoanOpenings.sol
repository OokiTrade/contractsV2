    ...
    PausableGuardian
{
    ...
    function getEstimatedMarginExposure(
        ...
        uint256 owedPerDay = newPrincipal.mul(interestRate).div(
            DAYS_IN_A_YEAR * WEI_PERCENT_PRECISION
        );

        uint256 interestAmountRequired = maxLoanTerm.mul(owedPerDay).div(
            1 days
        );
        ...
        if (value != 0) {
            return collateralTokenSent.add(value);
        }
    }
    ...
    function _borrowOrTrade(
        ...
        loanLocal.collateral = loanLocal.collateral.add(sentValues[4]).sub(
            amount
        ); // borrowingFee
        ...
        if (isTorqueLoan) {
            ...
            sentValues[2] = SafeMath.div(
                WEI_PRECISION * WEI_PERCENT_PRECISION,
                initialMargin
            );
        }
    ...
    function _initializeLoan(
        ...
        if (loanId == 0) {
            ...
            sloanLocal = loans[loanId];
            require(
                sloanLocal.active && block.timestamp < sloanLocal.endTimestamp,
                "loan has ended"
            );
    ...
    function _initializeInterest(
        ...
        if (maxLoanTerm == 0 && loanLocal.endTimestamp != 0) {
            ...
            .endTimestamp
            .sub(block.timestamp).mul(loanInterestLocal.owedPerDay).div(1 days); // block.timestamp < endTimestamp was confirmed earlier
        }

        uint256 owedPerDay = newPrincipal.mul(newRate).div(
            DAYS_IN_A_YEAR * WEI_PERCENT_PRECISION
        );
    ...
    function _getRequiredCollateral(
        ...
        if (loanToken == collateralToken) {
            collateralTokenAmount = newPrincipal.mul(marginAmount).divCeil(
