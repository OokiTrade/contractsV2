...
contract LiquidationHelper is State {
    function _getLiquidationAmounts(
    ...
    ) internal pure returns (uint256 maxLiquidatable, uint256 maxSeizable) {
        ...
        maxLiquidatable = maxLiquidatable.sub(
            collateral.mul(collateralToLoanRate).div(WEI_PRECISION)
        );
        maxLiquidatable = maxLiquidatable.mul(WEI_PERCENT_PRECISION).div(
        ...
        maxSeizable = maxLiquidatable.mul(
            incentivePercent.add(WEI_PERCENT_PRECISION)
        );
