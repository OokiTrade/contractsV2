...
library VolumeTracker {
  ...
  function convert(Observation memory last, uint32 blockTimestamp, uint128 tick) private pure returns (Observation memory) {
    return Observation({blockTimestamp: blockTimestamp, volCumulative: last.volCumulative + tick});
  }
  ...
  function write(Observation[65535] storage self, uint16 index, uint32 blockTimestamp, uint128 tick, uint16 cardinality, uint32 minDelta) public returns (uint16 indexUpdated) {
  ...
  /// @param cardinality populated elements
  function binarySearch(Observation[65535] storage self, uint32 target, uint16 index, uint16 cardinality) private view returns (Observation memory beforeOrAt) {
  ...
  /// @param index latest index
  /// @param cardinality populated elements
  function getSurroundingObservations(Observation[65535] storage self, uint32 target, uint16 index, uint16 cardinality) private view returns (Observation memory beforeOrAt) {
  ...
  function checkLastTradeTime(Observation[65535] storage self, uint32 time, uint32 secondsAgo, uint16 index) internal view returns (bool) {
  ...
  function observeSingle(Observation[65535] storage self, uint32 time, uint32 secondsAgo, uint16 index, uint16 cardinality) internal view returns (uint128 volCumulative) {
    if (secondsAgo == 0) {
      Observation memory last = self[index];
  ...
  function volumeDelta(Observation[65535] storage self, uint32 time, uint32[2] memory secondsAgos, uint16 index, uint16 cardinality) public view returns (uint128 volDelta) {
