...
contract MerkleDistributor is IMerkleDistributor, Ownable {
  ...
  function createAirdrop(address token_, bytes32 merkleRoot_, address source_, uint256 amount_) external override onlyOwner {
  ...
  function directClaim(uint256 airdropIndex, address account, uint256 amount) external onlyOwner {
  ...
  function adminClaim(uint256 airdropIndex, uint256 index, address account, uint256 amount, address sendTo, bytes32[] calldata merkleProof) external onlyOwner {
  ...
  function claim(uint256 airdropIndex, uint256 index, address account, uint256 amount, bytes32[] calldata merkleProof) external override {
    require(!isClaimed(airdropIndex, index), 'MerkleDistributor: Drop already claimed.');
