...
contract MerkleDistributor is IMerkleDistributor, Ownable {
  ...
  function _setClaimed(uint256 airdropIndex, uint256 index) private {
    uint256 claimedWordIndex = index / 256;
    uint256 claimedBitIndex = index % 256;
    claimedBitMap[airdropIndex][claimedWordIndex] =
      claimedBitMap[airdropIndex][claimedWordIndex] |
      (1 << claimedBitIndex);
  }
  ...
  function directClaim(
    ...
    if (source == address(this)) {
      ...
      require(
        IERC20(token).transferFrom(source, account, amount),
        'MerkleDistributor: Transfer failed.'
      );
    }
  ...
  function adminClaim(
    ...
    require(
      MerkleProof.verify(merkleProof, merkleRoot[airdropIndex], node),
    ...
    if (source == address(this)) {
      ...
      require(
        IERC20(token).transferFrom(source, sendTo, amount),
        'MerkleDistributor: Transfer failed.'
      );
    }
