...
V1_ITOKEN_MIGRATOR.transferOwnership(iETH_OLD.owner(),{'from': deployer})

extractor = TokenExtractor.deploy({'from': deployer})
balance = WETH.balanceOf(iETH_OLD)
print("Migrating amount: ", balance)
calldata = extractor.withdraw.encode_input(WETH, DAOGuardiansMultisig, balance)
Contract.from_abi("iETH_OLD", iETH_OLD, AdminSettings.abi).updateSettings(extractor, calldata, {'from': DAOGuardiansMultisig})

WETH.approve(iETH, WETH.balanceOf(DAOGuardiansMultisig), {'from': DAOGuardiansMultisig})
iETH.mint(V1_ITOKEN_MIGRATOR, balance, {'from': DAOGuardiansMultisig})
