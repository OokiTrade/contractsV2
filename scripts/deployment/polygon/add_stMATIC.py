...
for l in list:
    tokens.append(l[1])
tx_list.append([PRICE_FEED.address,False,PRICE_FEED.setPriceFeed.encode_input([STMATIC],[STMATIC_FEED.address])])
#add balancer swap impl to dex records
tx_list.append([BZX.swapsImpl(),False,Contract.from_abi("",BZX.swapsImpl(),DexRecords.abi).setDexID.encode_input("0x5281d5BbA9162B41320A2fc760F67B9e02fFd793")])
#set the tokens to be supported
tx_list.append([BZX.address,False,BZX.setSupportedTokens.encode_input([STMATIC],[True], True)])
...
tx_list.append([BZX.address,False,BZX.modifyLoanParams.encode_input([STMATIC_PARAMS,STMATIC_PARAM])])

print(interface.IMulticall3("0xcA11bde05977b3631167028862bE2a173976CA11").aggregate3.encode_input(tx_list))


# for tx in tx_list:
#     sTxn = safe.build_multisig_tx(tx[0], 0, tx[1], SafeOperation.CALL.value, safe_nonce=safe.pending_nonce())
