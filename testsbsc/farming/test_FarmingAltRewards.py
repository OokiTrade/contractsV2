...
def testFarming_alt_reward1(requireFork, tokens, tokenName, lpTokenName, pid, accounts, masterChef, bgovToken):
    # Precondition
    ...
    for account in [account1, account2]:
        ...
        bgovToken.approve(masterChef, 2**256-1, {'from': account})
        ...
        chain.sleep(60 * 60 * 24)
        chain.mine()
        masterChef.compoundReward(0,  {'from': account})
...
@pytest.mark.parametrize("tokenName, lpTokenName, pid", testdata)
def testFarming_alt_reward2(requireFork, tokens, tokenName, lpTokenName, pid, accounts, masterChef, bgovToken):
    # Precondition
    lpToken = tokens[lpTokenName]
    token = tokens[tokenName]
    account1 = accounts[1]
