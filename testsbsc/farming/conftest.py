...
@pytest.fixture(scope="class", autouse=True)
def bgovToken(accounts, BGovToken, Proxy1):
    bgovProxy = accounts[0].deploy(Proxy1, accounts[0].deploy(BGovToken), accounts[9])
...
@pytest.fixture(scope="class", autouse=True)
def masterChef(accounts, chain, MasterChef, iWBNB, iETH, iBUSD, iWBTC, iUSDT, bgovToken, Proxy1):
    ...
    masterChef = accounts[0].deploy(MasterChef)
    masterChefProxy = accounts[0].deploy(Proxy1, masterChef, accounts[9])
