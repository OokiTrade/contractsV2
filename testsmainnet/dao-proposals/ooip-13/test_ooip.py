...
def testGovernanceProposal(requireMainnetFork, accounts, DAO, TIMELOCK, BZX, OOKI, BZRX):
    ...
    exec(open("./scripts/dao-proposals/OOIP-13-omnibus-1year-extension/proposal.py").read())
    ...
    chain.mine()
    DAO_FUNDING = "0x37cBA8d1308019594621438bd1527E5A6a34B49F"
    beforeBalance = BZRX.allowance(TIMELOCK, DAO_FUNDING)
    DAO.execute(id, {"from": proposerAddress})
    BZRX_AMOUNT = 20400000e18

    afterBalance = BZRX.allowance(TIMELOCK, DAO_FUNDING)
    assert beforeBalance + BZRX_AMOUNT == afterBalance
    assert True