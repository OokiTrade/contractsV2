...
contract StakingVoteDelegator is
  StakingVoteDelegatorState,
  StakingVoteDelegatorConstants,
  PausableGuardian
{
   ...
   */
  function delegate(address delegatee) external pausable {
  ...
  function getPriorVotes(
    ...
    require(
      ...
      'Staking::getPriorVotes: not yet determined'
    );
  ...
  function _delegate(address delegator, address delegatee) internal {
    if (delegatee == delegator || delegator == ZERO_ADDRESS) return;
    ...
    if (srcRep != dstRep && amount > 0) {
      if (srcRep != ZERO_ADDRESS) {
        ...
        uint256 srcRepNew = srcRepOld.sub(
          (amount > srcRepOld) ? srcRepOld : amount
        );
  ...
  function _writeCheckpoint(
    ...
    uint32 blockNumber = safe32(
      block.number,
      'Staking::_writeCheckpoint: block number exceeds 32 bits'
    );
