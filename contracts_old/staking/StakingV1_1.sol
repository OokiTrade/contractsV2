...
contract StakingV1_1 is StakingState, StakingConstants, PausableGuardian {
  ...
  function _pendingSushiRewards(address _user) internal view returns (uint256) {
    uint256 pendingSushi = IMasterChefSushi(SUSHI_MASTERCHEF).pendingSushi(
  ...
  function _pendingAltRewards(
    ...
    uint256 _altRewardsPerShare = altRewardsPerShare[token].add(
      extraRewardsPerShare
    );
    if (_altRewardsPerShare == 0) return 0;

    IStaking.AltRewardsUserInfo
      memory altRewardsUserInfo = userAltRewardsPerShare[_user][token];
    return
      altRewardsUserInfo.pendingRewards.add(
        (_altRewardsPerShare.sub(altRewardsUserInfo.rewardsPerShare))
          .mul(userSupply)
          .div(1e12)
      );
  }
  ...
  function _depositToSushiMasterchef(uint256 amount) internal {
    ...
    uint256 sushiRewards = IERC20(SUSHI).balanceOf(address(this)) -
      sushiBalanceBefore;
    if (sushiRewards != 0) {
  ...
  function unstake(
    ...
    for (uint256 i = 0; i < tokens.length; i++) {
      ...
      uint256 pendingBefore = (token == LPToken)
        ? _pendingSushiRewards(msg.sender)
        : 0;
      ...
      if (
        token == BZRX && IERC20(BZRX).balanceOf(address(this)) < unstakeAmount
      ) {
      ...
      if (token == LPToken) {
        ...
        userAltRewardsPerShare[msg.sender][SUSHI] = IStaking
          .AltRewardsUserInfo({
  ...
  function _claimBzrx(
    bool restake
  ) internal returns (uint256 bzrxRewardsEarned) {
  ...
  function _discountVesting(
    ...
    if (bzrxRewardsVesting != 0) {
      bzrxRewardsVesting = bzrxRewardsVesting.sub(
        bzrxRewardsVesting.mul(multiplier).div(1e36)
      );
    }
    if (stableCoinRewardsVesting != 0) {
      stableCoinRewardsVesting = stableCoinRewardsVesting.sub(
        stableCoinRewardsVesting.mul(multiplier).div(1e36)
      );
    }
    ...
    )
  {
    ...
    (bzrxRewardsVesting, stableCoinRewardsVesting) = _discountVesting(
      bzrxRewardsVesting,
      stableCoinRewardsVesting
    );
    ...
    sushiRewardsEarned = _pendingAltRewards(
      ...
      (_totalSupplyPerToken[LPToken] != 0)
  ...
  function _addAltRewards(address token, uint256 amount) internal {
    ...
    altRewardsPerShare[token] = altRewardsPerShare[token].add(
      amount.mul(1e12).div(totalSupply)
    );
    ...
    returns (uint256 vBZRXWeight, uint256 iBZRXWeight, uint256 LPTokenWeight)
  {
    ...
    .mul(_startingVBZRXBalance - totalVested, 1e18).div(_startingVBZRXBalance); // overflow not possible
  ...
  function _calcIBZRXWeight() internal view returns (uint256) {
    return
      IERC20(BZRX).balanceOf(iBZRX).mul(1e50).div(IERC20(iBZRX).totalSupply());
  }
  ...
  function balanceOfStored(
    ...
    if (balance != 0) {
      vestedBalance = balance.mul(iBZRXWeightStored).div(1e50).add(
        vestedBalance
      );
    }
    ...
    if (balance != 0) {
      vestedBalance = balance.mul(LPTokenWeightStored).div(1e18).add(
        vestedBalance
      );
    }
  }
