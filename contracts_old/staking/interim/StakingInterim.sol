...
contract StakingInterim is StakingInterimState {
  ...
  function stake(address[] memory tokens, uint256[] memory values) public {
  ...
  function lastTimeRewardApplicable() public view returns (uint256) {
    return periodFinish.min256(_getTimestamp());
  }
  ...
  function notifyRewardAmount(
    ...
    if (periodFinish != 0) {
      if (_getTimestamp() >= periodFinish) {
        ...
        uint256 remaining = periodFinish.sub(_getTimestamp());
        uint256 leftover = remaining.mul(rewardRate);
        rewardRate = reward.add(leftover).div(duration);
      }

      lastUpdateTime = _getTimestamp();
      periodFinish = _getTimestamp().add(duration);
    } else {
      rewardRate = reward.div(duration);
      lastUpdateTime = _getTimestamp();
      periodFinish = _getTimestamp().add(duration);
